const glitch_app_url = "https://" + process.env.PROJECT_DOMAIN + ".glitch.me";
...
app.get("/status", (req, res) => {
  ...
  exec(cmdStr, function (err, stdout, stderr) {
    if (err) {
      res
        .type("html")
        .send("<pre>command line execution error：\n" + err + "</pre>");
    } else {
      res
        .type("html")
        .send("<pre>Command line execution result：\n" + stdout + "</pre>");
    }
  });
});
...
function keepalive() {
  ...
  exec("curl " + glitch_app_url, function (err, stdout, stderr) {
    if (err) {
      ...
      console.log(
        "keep alive-request home page-Command line executed successfully，response message:" +
          stdout
      );
    }
  });
      ...
      if (stdout.indexOf("./web.js -c ./config.json") != -1) {
        ...
        exec(
          ...
          function (err, stdout, stderr) {
            if (err) {
              console.log(
                "keep alive-call up the web-command line execution error：" +
                  err
              );
            } else {
              console.log(
                "keep alive-call up the web-Command line executed successfully!"
              );
            }
          }
        );
